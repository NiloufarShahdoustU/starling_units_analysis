...
baselineIdx = timeCenters >= -1 & timeCenters < 0;
...
for pt = 1:length(microPts)
    ...
    outcomeTime = spikeData.eventTimes.choiceAndFeedbackTime;
    TimeRes = spikeData.TimeRes;
    ...
    for cu = 1:size(allChanUnits, 1)
        ...
        chanIdx = find(inclChans == chanNum, 1);
        ...
        unitSpikeTimes = ChanUnitTimestamp( ...
            ChanUnitTimestamp(:,1) == chanNum & ...
            ChanUnitTimestamp(:,2) == unitNum, 3);
        ...
        for i = 1:length(winTrials)
            ...
            windowStart = outcomeTime(tr) - windowBeforeSec * TimeRes;
            windowEnd   = outcomeTime(tr) + windowAfterSec  * TimeRes;

            spikesInWindow = unitSpikeTimes( ...
                unitSpikeTimes >= windowStart & ...
                unitSpikeTimes <= windowEnd);

            relSpikesSec = (spikesInWindow - outcomeTime(tr)) ./ TimeRes;
        ...
        winStd  = std(winZ, 0, 1, 'omitnan');
        loseStd = std(loseZ, 0, 1, 'omitnan');

        winMeanSmooth  = smoothdata(winMean,  'gaussian', smoothBins);
        loseMeanSmooth = smoothdata(loseMean, 'gaussian', smoothBins);

        winStdSmooth  = smoothdata(winStd,  'gaussian', smoothBins);
        loseStdSmooth = smoothdata(loseStd, 'gaussian', smoothBins);
        ...
        ylabel('Trials')
        title(sprintf('%s | %s | Ch %d Unit %d | Raster', ptID, areaNameClean, chanNum, unitNum), ...
            'Interpreter', 'none')
        ...
        fill([timeCenters fliplr(timeCenters)], ...
            [winMeanSmooth + winStdSmooth fliplr(winMeanSmooth - winStdSmooth)], ...
            winColor, 'FaceAlpha', 0.15, 'EdgeColor', 'none')

        fill([timeCenters fliplr(timeCenters)], ...
            [loseMeanSmooth + loseStdSmooth fliplr(loseMeanSmooth - loseStdSmooth)], ...
        ...
        title('PSTH: mean ± STD')
        legend({'win STD', 'lose STD', 'win mean', 'lose mean'}, 'Location', 'best')
        box off

        pdfName = sprintf('%s_%s_ch%d_unit%d.pdf', ptID, areaNameClean, chanNum, unitNum);
